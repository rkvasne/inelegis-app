...
function parseRows(xml){
  ...
  let m;
  while((m=trRx.exec(xml))){
    let tr=m[1];
    if(/<TH>/i.test(tr)) continue;
    tr=tr.replace(/<TD\s*\/>/gi,'<TD></TD>');
    const tds=[];
    const tdRx=/<TD[^>]*>([\s\S]*?)<\/TD>/gi;
    let c;
    while((c=tdRx.exec(tr))){
      tds.push(stripTags(c[1]));
    }
    if(tds.length>=3){
      rows.push({norma:tds[0],excecoes:tds[1],crime:tds[2]});
    }
  }
  return rows;
}
function deriveCodigo(norma){
  ...
  if(/c[oó]digo penal|decreto-?lei\s*2\.848\/40/i.test(s)) return 'CP';
  const m=s.match(/(?:lei|decreto-?lei|lei\s*complementar)\s*(\d{1,5}(?:\.\d{3})*)(?:\/(\d{2}))?/i);
  if(m){
    const num=m[1].replace(/\./g,'');
    return `LEI_${num}`;
  }
  return 'LEI_DESCONHECIDA';
}
...
function build(){
  ...
  const trCount=(xml.match(/<TR>/gi)||[]).length;
  const rows=parseRows(xml);
  const items=rows.map(normalizeRow);
  const js=`;(function(){window.__INELEG_NORMALIZADO__=${JSON.stringify(items)};})();\n`;
  fs.writeFileSync(outJsPath,js,'utf8');
  console.log('TRs detectados:',trCount,'Linhas parseadas:',rows.length);
  console.log('Gerado:',path.relative(process.cwd(),outJsPath),'Itens:',items.length);
}
